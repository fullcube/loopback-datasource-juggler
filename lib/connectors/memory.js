...
Memory.prototype.replace = function save(model, data, options, callback) {
  var self = this;
  var id = this.getIdValue(model, data);
  var cachedModels = this.collection(model);
  var modelData = cachedModels && this.collection(model)[id];
...
Memory.prototype.replaceAttributes = function replaceAttributes(model, id, data, options, cb) {
  if (!id) {
    var err = new Error('You must provide an id when replacing attributes!');
    if (cb) {
      return cb(err);
    } else {
      throw err;
  ...
  var modelData = cachedModels && this.collection(model)[id];
  if (modelData) {
    this.replace(model, data, options, cb);
  } else {
    cb(new Error('Could not replace attributes. Object with id ' + id + ' does not exist!'));
  }
};
...
Memory.prototype.replaceOrCreate = function (model, data, options, callback) {
  ...
  this.exists(model, self.getIdValue(model, data), options, function (err, exists) {
    if (exists) {
      self.replace(model, data, options, function(err, data) {
        callback(err, data, { isNewInstance: false });
      });
    } else {
      self.create(model, data, options, function (err, id) {
