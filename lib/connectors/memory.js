...
Memory.prototype._create = function create(model, data) {
  ...
  if (this.collection(model)[id]) {
      throw new Error('Duplicate entry for ' + model + '.' + idName);
      return;
  }
...
Memory.prototype.create = function create(model, data, options, callback) {
  try {
    var id = this._create(model, data);
  } catch (e) {
    return callback(e);
  }
...
Memory.prototype.findOrCreate = function(model, filter, data, callback) {
  var self = this;
  var nodes = self._findAll(model, filter);
  ...
  if (nodes.length > 0) {
    ...
    this.saveToFile(id, function(err, id) {
      self.setIdValue(model, data, id);
...
Memory.prototype._findAll = function(model, filter) {
