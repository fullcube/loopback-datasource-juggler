  ...
  function doReplaceOrCreate(err, ctx) {
    ...
    if (connector.replaceOrCreate && isOriginalQuery) {
      ...
      Model.notifyObserversOf('before save', context, function(err, ctx) {
        ...
        // TODO: validateReplaceOrCreate needs to be set to true by default in model JSON
        if (Model.settings.validateReplaceOrCreate === false) {
          callConnector();
        } else {
          inst.isValid(function(valid) {
            if (!valid) {
              if (Model.settings.validateReplaceOrCreate) {
                return cb(new ValidationError(inst), inst);
              } else {
                // TODO(bajtos) Remove validateReplaceOrCreate:undefined in v3.0 ?!
                console.warn('Ignoring validation errors in replaceOrCreate():');
                console.warn('  %s', new ValidationError(inst).message);
                // continue with replaceOrCreate
              }
            }
            callConnector();
          }, update);
        ...
        function callConnector() {
          ...
          Model.notifyObserversOf('persist', context, function(err) {
            if (err) return done(err);
            if (connector.replaceOrCreate.length === 4) {
              connector.replaceOrCreate(Model.modelName, update, options, done);
            } else {
              connector.replaceOrCreate(Model.modelName, update, done);
            }
          });
        }
        function done(err, data, info) {
      ...
      Model.findOne({where: ctx.query.where}, opts, function (err, inst){
        ...
        if (inst) {
          inst.replaceOrCreate(data, options, cb);
        } else {
          Model = self.lookupModel(data);
          var obj = new Model(data);
          obj.save(options, cb);
 ...
 * Equivalent to `replaceAttributes({name: value}, cb)`
 *
 * @param {String} name Name of property
 * @param {Mixed} value Value of property
 * @param {Function} cb Callback function called with (err, instance)
 */
DataAccessObject.prototype.replaceAttribute = function replaceAttribute(name, value, options, cb) {
  var data = {};
  data[name] = value;
  return this.replaceAttributes(data, options, cb);
};
 ...
 * @param {Object} [options] Options for replaceAttributes
 * @param {Function} cb Callback function called with (err, instance)
 */

DataAccessObject.prototype.replaceAttributes = function replaceAttributes(data, options, cb) {
  var connectionPromise = stillConnecting(this.getDataSource(), this, arguments);
  if (connectionPromise) {
    return connectionPromise;
  }
  ...
  var Model = this.constructor;
  var connector = inst.getConnector();
  assert(typeof connector.replaceAttributes === 'function',
    'replaceAttributes() must be implemented by the connector');
    ...
    inst.isValid(function (valid) {
      ...
      inst.trigger('save', function (saveDone) {
        inst.trigger('update', function (done) {
          ...
          function replaceAttributesCallback(err) {
            if (err) return cb(err);
            var ctx = {
              Model: Model,
              data: context.data,
            ...
            Model.notifyObserversOf('loaded', ctx, function(err) {
              ...
              // By default, the instance passed to replaceAttributes callback is NOT updated
              // with the changes made through persist/loaded hooks. To preserve
              // backwards compatibility, we introduced a new setting updateOnLoad,
              // which if set, will apply these changes to the model instance too.
              if(Model.settings.updateOnLoad) {
          ...
          Model.notifyObserversOf('persist', ctx, function(err) {
            if (connector.replaceAttributes.length === 5) {
              connector.replaceAttributes(model, getIdValue(inst.constructor, inst),
                inst.constructor._forDB(context.data), options, replaceAttributesCallback);
            } else {
              connector.replaceAttributes(model, getIdValue(inst.constructor, inst),
                inst.constructor._forDB(context.data), replaceAttributesCallback);
            }
          });
        }, data, cb);
      }, data, cb);
